context('Go through calculator', () => {
  it('', () => {
    ...
    cy.get('ui5-wizard-step[title-text="Base Configuration"]:visible').within(() => {
      ...
      cy.document().within(() => {
        cy.CostShouldBe('nodes-cost', '1036.80 CU')
        cy.CostShouldBe('total-capacity-units', '1036.80 CU')
        cy.CostShouldBe('total-in-currency', '1099.01 €')
      })
    ...
    cy.get('ui5-wizard-step[title-text="Additional Configuration"]:visible').within(() => {
