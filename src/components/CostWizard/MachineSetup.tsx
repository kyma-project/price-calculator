...
export default function MachineSetupForm(props: Props) {
  ...
  const machineTypeDefaultValue = config.nodeConfig.MachineTypes[0];
  const machineTypeDefaultValueVMSize =
    machineTypeDefaultValue.VMSizeOptions[0];
    ...
    for (let i = 0; i < machineSetup.length; i++) {
      if (machineSetup.at(i)?.visible) {
        const updatedCost = calculateNodeConfigCosts({
          timeConsumption:
            machineSetup.at(i)?.timeConsuption ?? timeConsumptionDefaultValue,
          vmMultiplier:
            machineSetup.at(i)?.VMSize.multiple ??
            machineTypeDefaultValueVMSize.multiple,
          minAutoscaler:
            machineSetup.at(i)?.minAutoscaler ?? autoscalerMinValue,
          machineTypeFactor:
            machineSetup.at(i)?.machineType.multiple ??
            machineTypeDefaultValue.multiple,
        });
