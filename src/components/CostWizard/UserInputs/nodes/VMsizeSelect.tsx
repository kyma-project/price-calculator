...
export default function VMsizeSelect(props: Props) {
  const baseConfigOptions = config.nodeConfig.MachineTypes;
  ...
  const VMSizeIndexMachineType = baseConfigOptions
    .filter(
      (item) =>
        typeof machineSetup[props.nodeIndex] === 'undefined' ||
        item.value === machineSetup[props.nodeIndex].machineType.value,
    )
    .flatMap((item) => item.VMSizeOptions);
